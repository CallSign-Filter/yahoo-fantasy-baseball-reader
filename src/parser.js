...
function getDataCleanupParseOutput() {
  getAllTeamsData()
    ...
    .then((data) => parseWRandRBPositionWithFlex(data))
  ...
  try {
    ...
    if (yahoo.yfbb.CREDENTIALS) {
      ...
      for (let i = 1; i <= yahoo.yfbb.NUMBER_OF_TEAMS; i++) {
        const team = await yahoo.yfbb.getCurrentRosterForTeam(i);
        teams.push(team);
      }
...
const parseAllData = async (data) => {
  ...
  for (const team of data) {
    ...
    const stats = await yahoo.yfbb.getPlayerStatsData(playersStringArr.join(","));
    const points = stats.fantasy_content.league.players; // .player.player_points.total;
